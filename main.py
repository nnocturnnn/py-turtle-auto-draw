...
if __name__ == '__main__':
    ...
    if bitmap.shape[0] > int(NSScreen.mainScreen().frame().size.height):
        bitmap = cv2.resize(bitmap, (int(bitmap.shape[1] * (
            (int(NSScreen.mainScreen().frame().size.height) - 50) / bitmap.shape[0])), int(NSScreen.mainScreen().frame().size.height) - 50))
    drawBitmap(bitmap)
